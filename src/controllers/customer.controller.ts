...
import { catchResponse, errorResponse, generateToken, hashPassword, paginate, removeUploadedFiles, successResponse } from "../utils";
...
export const createCustomer = async (req: Request, res: Response): Promise<any> => {
    ...
    const { firstName, lastName, email, phone, password, gender, address, city, state, pincode, aadhaarNumber, panNumber, verificationStatus, remarks } = req.body;
...
export const getAllCustomer = async (req: Request, res: Response): Promise<any> => {
    ...
    try {
        ...
        const result = await paginate({
            ...
            searchFields: ['firstName', 'lastName', 'email', 'address', 'city', 'state'],
